...
- **The engine:** `COS_LENS_GIST_ENGINE` and `COS_LENS_GIST_MODEL`, then the saved choice, then Claude Sonnet when the Claude CLI is installed (off otherwise). An explicit engine never falls back to another; a mistyped one, or a saved file that cannot be read, is off.
- **Measured with the shipped prompt** on real replies: Claude Sonnet 2 to 3.5 s on about 1.2k tokens; Codex `gpt-6-luna` and `gpt-6-sol` about 3 s on about 12.6k (Codex's own harness); Cursor `grok-4.7-low-fast` and `composer-2.5-fast` about 11 to 12 s on about 11 to 12k; Ollama `qwen3.8:27b` 14 s cold. Claude Haiku timed out at 30 s twice.
