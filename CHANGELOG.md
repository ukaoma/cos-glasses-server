...
Two things from 2026-09-12: a dictated prompt that ended in a sentence nobody said, and held voices that could only be named one meeting at a time.

- A prompt no longer keeps a trailing sentence the speaker did not say. Chris's prompt closed with "I'm going to go to the bathroom" after he had stopped talking: the interactive path runs whisper without VAD on purpose (so a short prompt is never trimmed), and a decoder handed trailing silence plus a vocabulary prompt finishes the sentence it thinks it heard. `prompt-tail-guard.ts` checks the tail of every prompt chunk against the word timings whisper already returns and the Silero speech windows. A last sentence is dropped only when it is a known filler ("thanks for watching", the bathroom line, "see you next time", "subtitles by", …) or when it starts at least a second after the last speech ended AND whisper's own mean word confidence for it is under 0.45. At most two sentences go; a real closing phrase at normal confidence stays; the whole chunk is refused only when the VAD heard no speech at all and whisper was unsure of every word (Miles: "as long as we don't end up cutting the final portion of a user's prompt"). Every drop logs as `[prompt-draft] tail_drop` with its reason and the text. With the VAD model not loaded the guard reports unknown, never silence, and only the lexicon applies.
- Held voices are grouped by who they sound like, across every meeting in the window. `GET /api/voice/held-groups` clusters every retained unidentified sample with the same mutually-coherent rule that guards profile corrections (the chunk-embedding bank already holds each sample's voiceprint, so the ordinary listing decodes no audio; a sample the bank lacks is decoded inside a one-second budget, cached, and finished in the background) and answers with `groups` — members, meetings, coherence, the sample to play first, and `suggestion`, the enrolled profile that vouches for the group: at least two of its samples above the identifier's 0.55 floor, one of them from an anchored source (manual, Fireflies, a G2 enrolment, a correction, a named held group — never a bulk `ext-retroactive` session enrol, which on the live store had written one household voice into two people's profiles), scored on the second-best sample so one polluted sample cannot vouch alone, `high` at the live identifier's own 0.72 auto-enrol bar and `likely` above the floor, with `agreeing`/`of` counts — plus `loose`, the samples that cohere with nothing (the random artifacts). Exact duplicate chunks (a recording started twice banks every chunk under two ids; 49 such pairs on the live store) fold into one sample before grouping, so a doubled recording is not a two-sample voice; members still carry every wav. `POST /api/voice/held-groups/enroll { name, members }` names a set as one person, appending when the name already has a profile, which is how a voice that missed in a new room heals; it re-checks coherence on the server, writes only the coherent core (diverse subset, max 20, source `ext-group`), removes only those wavs, and leaves anything that was not that voice held. A set that agrees on nothing is refused (409 `incoherent`) rather than enrolled as whichever stranger came first. `POST /api/voice/held-groups/discard { members }` throws samples out. `GET /api/voice/ext-audio` and `POST /api/voice/enroll-ext` are unchanged; COS Control 0.5.219 builds Add-a-voice on the new routes.
