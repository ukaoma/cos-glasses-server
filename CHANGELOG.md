...
- **A chain, for resilience:** `claude:sonnet,codex:gpt-5.6-terra` (env, or `PUT {"chain": ...}`) asks the next engine when one fails, is cooling down, or reports a spent usage limit, which opens that engine's breaker at once. Running low on one plan no longer turns the summary off. Opt-in only: a chain sends the reply to every provider in it. An answer any engine wrote is reused for the whole chain. Tested live: with Claude failing, Codex Terra answered in 4.9 s end to end.
- **Measured with the shipped prompt** on real replies: Claude Sonnet 2 to 5 s on about 1.2k tokens; Codex `gpt-5.6-terra` (the Codex default, the peer of Sonnet) 3.2 to 3.7 s on about 11.4k, `gpt-6-luna` and `gpt-6-sol` about 3 s on about 12k (Codex's own harness); Cursor `grok-4.7-low-fast` and `composer-2.5-fast` about 11 to 12 s on about 11 to 12k; Ollama `qwen3.8:27b` 14 s cold. Claude Haiku timed out at 30 s twice.
...
- **How it stops:** answers cached on disk per reply, engine and model; a daily cap of 150 answers (`COS_LENS_GIST_DAILY_CAP` or the saved `dailyCap`) and twice that in attempts; a failed reply refused for 10 minutes, then 6 hours; a breaker per engine (3 failures in a row, 30 minutes, then one trial call); at most 2 running and 4 waiting, with the cap, breaker and drain checked again when a queued call gets its turn. The route sits outside the request lease, so a slow model never holds up Update Server, and it answers `maintenance` during a drain.
- **Visible:** a `g2-lens-gist` row in the token audit for every call, `<data>/lens-gist-runs.jsonl`, and `lens_gist` on `/api/health` (engine, model, calls today, cap, open breakers; no error text on the unauthenticated slice).
...
- Checks: two /qa rounds (four reviewers, then one on the fixes). 29 mutation cases, all killed from a green baseline. Full suite 5,190 passed, 2 skipped (343 files); typecheck clean; `npm audit --omit=dev` 0 vulnerabilities.
