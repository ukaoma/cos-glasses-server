...
- **Until the COS Glasses build with the cards ships**, a held request can only be answered at the Mac (it is handed back the moment you touch the keyboard or mouse) or it waits out the deadline. Set `COS_PERMISSION_BROKER=0` until then if that wait matters.
...
- Tests, against a real express server and real sockets, wired as `index.ts` wires them: every fast-path reason answers `{}` in under 100 ms (a Bash request with the desk active, including one with the real `ioreg`; the lens not live or silent 61 s; a Cursor envelope; the kill switch; a drain; approvals off; a plan approval; an unreadable desk); a question answered from the API reaches the hook as the exact `updatedInput` shape; an approval allowed and denied; desk return, a drain, and the deadline each answer `{}`; a hook that hangs up is released at once and a late answer gets 410; two racing answers, one wins and the winner's retry replays; retraction when the session's own tool runs and not when a parallel one does; answer validation; the hook token goes through the constant-time comparison; the row reads `waiting`/`question` with `pending_question_id` while held and clears after the answer. The real hook script: the existing round trip now asserts the URL and the unwrapped `tool_input`; the script prints exactly the question and deny decisions from a real broker; no lens live or the kill switch prints nothing; an old script against this server (401) and this script against a 6.51 server (404) print nothing. Gate: 47 new cases (`broker-*`, `store-*` except `store-day-counts-domain`, `derive-*`, `rows-*`, `liveness-*`, `hook-script-posts-old-url`), run against a green baseline. `store-request-id-dropped-on-reannounce` survived the first draft of the tests (the broker's own listener re-attached the id and hid the reducer's loss of it), and a reducer-only test was added for it.
