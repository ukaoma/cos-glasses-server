...
A Messages run now keeps a readable trail: the agent's own words between steps, each step as a verb and a short target, how it ended, and Claude Code's own step lines. Server only; the glasses read it from their next release (Phase 2). A client that does not read it sees no difference.
...
- **Transport.** A new event type, `trail`: its data is one entry plus `trailSeq`, a dense 1..n per job assigned inside the serialized journal append. The job `eventSeq` is shared with chunks and activity, so it is never the trail's order. `snapshot.trail` keeps the newest 200 entries and at most 32,000 characters of text across them, is rebuilt from the journal on restart, and is absent on a job with no trail rows. `QUERY_JOB_SCHEMA_VERSION` stays 1. Trail events count toward the 256-event replay ring; after a replay gap a client rebuilds the page from the snapshot. The legacy `/api/query` route and openai-compat get no trail. The trail never goes on the display bus.
- **Privacy.** Every field is redacted at the journal boundary, before it is written: first the private-material and secret patterns (new `redactSecretText`, which keeps newlines and does not hide ordinary narration the way `looksOpaqueSecret` does), then the store's credential and local-path patterns, then the caps (prose 2,000 characters, target 80, detail 40, step line 160). Redaction runs before the cut, so a secret is never half shown.
- **The user's opt-out holds.** `activityToolMode` `off`: no tool steps and no step outcomes, only prose and step lines. `status`: steps with verb and short target, and a failure shows a derived token (`exit 1`, `denied`, `error`), never output text. `preview`: the full grammar.
- **Capability and rollback.** `/api/models` reports `capabilities.durableQueryJobs.trail`: `true`, or `false` when jobs are off or `COS_MESSAGES_TRAIL=0`. That flag, listed in `managed-runtime-contract.json`, is the rollback that needs no downgrade: the job runner then hands the bridges no trail callback, and every stream, snapshot and answer is the 6.51.0 one. COS Control drops a hand-set flag its allowlist does not name, which leaves the trail on (the default). A 6.51.0 server reading a journal this build wrote counts the `trail` rows as malformed and loads every job; checked against the real 6.51.0 store: `malformedRows` 3, the completed job's snapshot identical apart from `trail`. Rolling forward again reads them with `malformedRows` 0, and a job that was running when the rollback happened still ends `interrupted`.
- Tests: each engine's measured line shapes through its reader, and the same stdout through the real Claude, Codex, Cursor and Ollama bridges with and without a trail consumer (argv, stdin, every callback, the history writes and the Claude cancel path identical; only the trail differs); coalescing; redaction (secrets and paths absent from the journal bytes, narration and newlines kept); the three modes end to end; `trailSeq` dense among chunk, tool status and activity events; a restart keeps the trail with `malformedRows` 0; the 6.51.0 allowlist simulation, and a rollback then roll forward mid-run; answer, history and display events identical with the trail on and off; the display bus never carries a trail string; the capability and the flag. The one changed pin is `health-query-jobs.test.ts`, whose capability shape gains `trail`. Gate: 24/24 new `trail-*` cases, each kill traced to the test that names the behavior.
...
- **What it does.** When a Claude session asks a question (the AskUserQuestion card) or wants approval for a tool, and you have been away from the Mac for 90 s while a client that can answer is polling for questions, the server holds the session's own PermissionRequest hook and lists the request at `GET /api/session-questions`. An answer posted to `POST /api/session-questions/:id/answer` goes back through the hook. A question returns `allow` with the original input plus `answers`: each question's original text, byte for byte, mapped to a list of labels, with an "Other" answer as its own entry exactly as written (Claude Desktop's own shape; the model reads "The user answered"). An approval returns allow once, or deny with the message "Denied from COS glasses". Never a permission rule: `updatedPermissions` and Claude's suggestions are never returned. Canaries on Claude Code 2.1.278 proved each shape: questions in interactive and background sessions, allow and deny for Bash. Print-mode runs have no AskUserQuestion tool, so this is for sessions only.
- **Only when away and wearing.** Everything else gets `{}` at once, which is the Mac's own dialog, exactly as before this release. The gates, cheapest first: the switch is on; no maintenance drain; a Claude request (a Cursor payload is never held); AskUserQuestion, or approvals are on; never ExitPlanMode (its dialog picks a permission mode, and allow once is not a plan approval); a client that polled `GET /api/session-questions` in the last 60 s; and the desk idle for 90 s, read by the server itself with one bounded `ioreg` (about 30 ms). Each of those answers in under 100 ms, and the switch and the drain answer without reading the desk at all.
- **The liveness signal is the questions poll itself**: an authenticated `GET /api/session-questions` (X-Cos-Token, checked by the route itself in constant time as well as by the /api gate; the hook token never counts) within the last 60 s. Only a client that asks for questions can answer one. The client-instance claim was rejected on review: every COS Glasses copy since 6.9.505 posts it every 15 s, including the ones with no question UI, so counting it would have held questions and approvals that nobody could see. The display stream (an open socket is not proof), client diagnostics (lossy) and meeting chunks (only while recording) were rejected too. A copy whose timers are frozen cannot poll, and could not show a question either.
- **A held request ends exactly once**: an answer; the desk becoming active again (checked every 1.5 s), which hands it straight back to the Mac's dialog; the deadline (110 s by default, counted from when the hook started, and never at or above the hook's own 125 s); the hook hanging up; the session moving on without us (its own tool ran or was denied because the dialog was answered at the desk, or the turn ended: Stop, StopFailure, SessionEnd, a new prompt, a fresh SessionStart); a drain, or the server shutting down. A late answer gets 409 `already_answered` (with what was chosen), 409 `handed_to_desk`, 409 `expired`, or 410 `hook_gone`. The first answer wins, and a retry with the same `clientAnswerId` gets the same result back. The server checks that the hook's connection can still take the reply before it accepts an answer. Answers are validated: every question answered, labels exactly one of that question's options, one entry for a single-select question, "Other" up to 2,000 characters.
- **The hook's door is outside `/api`**: `POST /hooks/permission-requests/ask`, checked against the per-install hook token in constant time like the 6.51.0 Cursor route, and mounted whatever `COS_THREAD_ATTACH_ENABLED` says. The hook script posts there now. The old `/api/permission-requests/ask` was never served: the /api gate answered the hook token 401, so every such request fell back to the dialog. Mixed versions fail safe: an old script against this server gets 401, this script against an older server gets 404, and both print nothing, so the dialog shows. The new script reaches `~/.cos-glasses/bin` through `--hooks install` (Control's "hooks outdated" banner); the settings entries do not change.
- **Rows.** An AskUserQuestion now stays `waiting_kind: question` through its PermissionRequest. It used to become `permission`, which clears only on an exact input match, and the answered tool runs with `answers` added to its input, so the row could stay waiting. A held item's id rides the agent-sessions and claude-sessions rows as `pending_question_id` (a question) or `pending_permission_id` (an approval); both are ids from `GET /api/session-questions`, and neither goes on the live session stream. A wait the broker is holding stands even when the registry moves meanwhile, and the id is dropped however the item ends. A late copy of a request that was already answered does not leave the row waiting; the same question asked again later does.
- **Switches** (the names reserved in 6.48.0 and listed in `managed-runtime-contract.json`). `COS_PERMISSION_BROKER`: `0`, `false` or `off` answers `{}` to everything, with no reinstall; `questions` holds questions only; unset or any other value holds both (Miles, 2026-09-19). It is also off whenever the session hooks are off (`COS_SESSION_HOOKS`). `COS_PERMISSION_BROKER_DESK_IDLE_S` (default 90; the script reads the same value) and `COS_PERMISSION_BROKER_TIMEOUT_S` (default 110, clamped to 5 through 120). COS Control's environment allowlist does not carry these three yet, so a hand-set value is dropped at Control's next Update Server and the defaults apply; the next Control adds them.
- **Inert until a client that polls for questions exists.** COS Glasses 6.9.511 and earlier, and COS Control, never call `GET /api/session-questions`, so with them every permission request is answered `{}` at once and nothing changes, whatever the switches say. The broker starts holding requests only once a client with the question cards polls. A future client that polls without being able to answer (a Control pet that only shows a count, say) would count as live; such a client must not poll this route.
- **Health.** `/api/health` gains `permissionBroker`: enabled, mode, pending, the counters (parked, answered, handed_to_desk, expired, hook_gone, retracted, drained, and fast_path by reason) and the last fast-path reason. Counts only. The log carries ids and counts, never question or command text.
- Tests, against a real express server and real sockets, wired as `index.ts` wires them: every fast-path reason answers `{}` in under 100 ms (a Bash request with the desk active, including one with the real `ioreg`; no questions poll; a Cursor envelope; the kill switch; a drain; approvals off; a plan approval; an unreadable desk); a question answered from the API reaches the hook as the exact `updatedInput` shape; an approval allowed and denied; desk return, a drain, and the deadline each answer `{}`; a hook that hangs up is released at once and a late answer gets 410; two racing answers, one wins and the winner's retry replays; retraction when the session's own tool runs and not when a parallel one does; answer validation; the hook token goes through the constant-time comparison; the row reads `waiting`/`question` with `pending_question_id` while held and clears after the answer. Liveness, with the real module: a live 6.9.511 claim and no questions poll takes the fast path; a poll 59 s old parks; one 61 s old does not, and a fresh one parks again; a poll with the hook token (or none) is refused 401 and never counts, with or without the /api gate in front; exactly 60 s still counts and 60.001 s does not. The real hook script: the existing round trip now asserts the URL and the unwrapped `tool_input`; the script prints exactly the question and deny decisions from a real broker; no questions poll (with and without a live 6.9.511 claim) or the kill switch prints nothing; an old script against this server (401) and this script against a 6.51 server (404) print nothing. Gate: 52 new cases (`broker-*`, `store-*` except `store-day-counts-domain`, `derive-*`, `rows-*`, `liveness-*`, `hook-script-posts-old-url`), run against a green baseline. The liveness cases: the poll not recorded, a hook-token poll counted, a claim counted, the 60 s window at its boundary and doubled, and the poll clock moving backwards. `store-request-id-dropped-on-reannounce` survived the first draft of the tests (the broker's own listener re-attached the id and hid the reducer's loss of it), and a reducer-only test was added for it.
