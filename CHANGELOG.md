...
- **The referee never hands the ring over mid-meeting.** QA of 6.50.3 (with glasses 6.9.507 already on the phone): the copy that yields stops its recording, which is how a duplicate ends, so reopening COS on a blank phone page during a meeting (the 2026-09-18 08:15 symptom) would have ended the meeting within 15 s, with the new window not recording. Now, while a meeting chunk has arrived in the last 30 s (`CLIENT_INSTANCE_CAPTURE_LIVE_MS`, read from the newest `lastActivityAt`, stamped on chunk arrival before any filtering, so silence still counts), the owner keeps the ring however quiet its claims (a hidden WebView throttles its timers) and a newer copy is told to wait; the ring moves on the first claim after the meeting stops. The answer carries `captureLive`. This protects glasses 6.9.507 as soon as the server updates.
...
- Tests: the hold executed (held while live, held while the owner's claims lapse, released after) and over HTTP (`captureLive` in the answer; a dual-stack server where `127.0.0.1` and `::1` are two devices with two owners; a future boot is a 400). Gate: `referee-hands-over-mid-meeting`, `referee-future-boot-accepted`, `referee-one-owner-for-all-devices`, `referee-capture-probe-unwired` added; 8/8 referee cases.
