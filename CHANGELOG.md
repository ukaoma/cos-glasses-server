...
One derivation everywhere, a Desktop turn ends when the engine or the transcript says it ended, a queued follow-up drains on Stop instead of the 30 s idle window, and the live feed carries the same state every row does. No client change is required; Control 0.5.232 and EHPK 6.9.481 read the new keys, and 6.48.0 consumers ignore them.

- A queued follow-up is delivered when the turn ends. The hooks' `Stop` kicks the drain through one in-flight guard shared with the 20 s timer, so a Stop landing mid-sweep folds into exactly one follow-up pass and a sentence is never delivered twice; a kick is spent only when a queue names that session (full id or the registry's 8-character form); a `SubagentStop` that closes the last sub-agent of a finished turn kicks the same way. Measured before: 35 s or more from the turn's end. Target now: under 2 s.
- The attach gate reads the engine's own end of turn. With `COS_SESSION_HOOKS` on, a foreign Desktop holder whose newest hook event is a `Stop` within 2 s of the transcript's newest write reads idle at once (the 146 ms of bookkeeping rows Claude writes after Stop sit inside that grace) instead of after the 30 s window. Null, unknown, Codex and a far-future clock stay strict; the clause narrows a `working` verdict and never lifts a doubt.
- The transcript's own rule for "the turn ended": the newest assistant record's `stop_reason` is terminal (end_turn, stop_sequence, max_tokens, refusal) with no `tool_use` still awaiting its result, a `result` row, or a user interrupt; a new prompt reopens it, a `<task-notification>` included; meta and compact rows do not. A Desktop transcript writes no `result` row (0 of 30 newest on the release Mac), so the old status-draft rule could only end a `claude -p` run and every Desktop Continue waited the idle window.
- Every `status` draft on the session stream carries `agent_state`, `state_source`, `state_since` and, when known, `waiting_kind`, `waiting_detail`, `failure`, as extra fields on the existing kind; `state` keeps its vocabulary (running and waiting read `working`, idle and failed `idle`, ended `done`). Every change of a session's hook state is a live status draft, so a client's state line moves on the engine's events. A shipped lens renders the frames exactly as before (its parser keeps `seq` and strips unknown fields). `COS_SESSION_HOOK_SSE=0` restores 6.48.0's frames.
- Reconnect without a reseed: each streamed session keeps a ring of its last 500 events with a per-session `cursor` (also the SSE `id:` line), the active turn is kept whole up to 2,000, the ring outlives the last subscriber by five minutes, and `?after=<cursor>` replays what was missed; a cursor past the ring reseeds. `?seed=turn` opens a session at its current prompt and every step since, oldest first, for clients that read top to bottom. The default seed is unchanged.
- `queued_turns` on every `/api/agent-sessions` row (list, search, detail) and every `/api/claude-sessions` peer: the waiting follow-ups on that thread, omitted at 0.
- Two 6.48.0 defects: a tab adopted mid-turn read idle from a `PostToolUse` with no prompt seen (Claude 2.1.272 reloads hooks when the settings change, so open tabs report without a restart); a main-thread tool event now opens the turn. `GET /api/session-hooks/runs` listed an interactive tab as a run with no end; runs carry the registry `entrypoint` (`sdk-cli` for `claude -p`, `claude-desktop`, `cli`) and tabs are listed only with `?all=1`.
- Two defects the live proof found in the follow-up path, both older than 6.48.0. The drainer's loopback delivery sent a turn body without `epoch` and `targetKey`, which the turns route has required since the binding hardening, so every drained follow-up was refused `invalid_request`, refunded and held for its whole TTL: it now sends the epoch the attach minted, the server's own target key and the attach's `boundTo`. And the 30 s idle backstop delivered a follow-up INTO a turn whose tool was still running (a 45 s tool leaves the transcript untouched for 45 s): positive evidence that the turn is open (a tool awaiting its result, a prompt newer than the last reply, or the hooks saying the turn has not stopped, each bounded to the last 30 minutes) now holds ahead of the backstop. A queued turn whose `cosSessionId` the attach route would refuse is refused at enqueue instead of sitting in the queue. Measured on the release Mac: a follow-up queued during a 45 s tool held, then landed 128 ms after the Stop hook, once, in the holder's own transcript.
- `/api/health` `sessionHooks.drain` reports the kick's passes, kicks and folds.
