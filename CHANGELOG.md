...
- Every automatic action can be undone. High confidence merges and content-anchored splits of long recordings apply automatically with Undo or Revert; an undone merge is never repeated by automation. Lower confidence pairings become suggestions to confirm or dismiss. Recordings made before the engine was installed only ever become suggestions.
...
- New routes: fireflies-key set, status, check and delete; meeting-import fireflies run, status and settings; meeting-suggestions list, accept, confirm and dismiss; meeting-actions list, revert and revert-all; meeting-engine status and mode. The contract is in docs/meeting-merge-contract.md.
