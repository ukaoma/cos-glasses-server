...
- **`POST /api/agent-sessions/:provider/:threadId/cancel`**, body `{"clientCancelId": "..."}`, registered whether or not thread attach is on. `202 {"target": "cos_turn", "state": "stopping", "turnId", "queuedHeld"}` or `202 {"target": "desk_run", "effective": "next_tool_call", "queuedHeld", "settledPermissions"}`; otherwise `409 {"reason", "reasonCopy"}` with `not_running` ("Nothing running now."), `cancel_unsupported`, `hooks_outdated` ("Install hooks in COS Control, then retry.") or `hooks_disabled`. The same `clientCancelId` gets the same answer again. Every cancel is a line in `data/session-cancel.jsonl`.
