...
- Held voices are grouped by who they sound like, across every meeting in the window. `GET /api/voice/held-groups` clusters every retained unidentified sample with the same mutually-coherent rule that guards profile corrections (the chunk-embedding bank already holds each sample's voiceprint, so the ordinary listing decodes no audio; a sample the bank lacks is decoded inside a one-second budget, cached, and finished in the background) and answers with `groups` — members, meetings, coherence, the sample to play first, and `suggestion`, the enrolled profile the group sounds like: `high` at the live identifier's own 0.72 auto-enrol bar, `likely` above its 0.55 floor, nothing below it — plus `loose`, the samples that cohere with nothing (the random artifacts). `POST /api/voice/held-groups/enroll { name, members }` names a set as one person, appending when the name already has a profile, which is how a voice that missed in a new room heals; it re-checks coherence on the server, writes only the coherent core (diverse subset, max 20, source `ext-group`), removes only those wavs, and leaves anything that was not that voice held. A set that agrees on nothing is refused (409 `incoherent`) rather than enrolled as whichever stranger came first. `POST /api/voice/held-groups/discard { members }` throws samples out. `GET /api/voice/ext-audio` and `POST /api/voice/enroll-ext` are unchanged; COS Control 0.5.219 builds Add-a-voice on the new routes.
