...
- **Every process a turn started is stopped.** The root process group is signalled while Node has not reaped the leader, including when the first process-table read fails. Every process group a tree member sits in is remembered with its leader's start time, so members re-parented to launchd are still owned, signalled and counted. On a cancel or a timeout, `reaped: true` is never reported while an owned process survives; a natural exit settles as soon as the CLI itself is reaped, with no tree probe, so it makes no claim about background jobs a tool left behind. A failed or malformed read still latches doubt and fences.
...
- **Cancel during the live hand-off works.** It is latched and answered 202 `stopping` instead of 409 `cancel_failed`. If nothing reached the session the turn settles `turn_cancelled`; if it reached, the desk halt marker is written, and re-armed once if the first prompt that session starts after the hand-off is exactly the delivered one (see the /qa round below). Codex answers `cancel_unsupported`. Replaying the same `clientCancelId` returns the final outcome.
- **Fences left by a cancel release themselves.** A fence is released and logged only when its reason is `cancelled`, it was set at the ambiguous-delivery site, and every recorded child is confirmed gone. Only `ps` answering "no such process" confirms it; a `ps` that times out, cannot start or is missing keeps the fence. This clears threads locked by 6.53.1's false "may not have been delivered".
...
- **The session hook is about 40% faster and never breaks a large tool input.** The halt check, tool-name check and session-id parse run in shell on the first 4 KiB (read with `dd`: macOS `head -c` over-reads a pipe), and stdin is drained on every path of every event, so a 2.5 MB Write (or a large permission request) no longer hits EPIPE. There is no temp file, so a new prompt always clears its session's marker. Median per tool call: about 29 ms to 17 ms quiet, 35 to 42 ms to about 23 ms under load. Those were measured on the pre-QA script; the /qa round's drain changes leave that path as it was (40 runs each, interleaved, on a quieter Mac: 11.2 and 10.9 ms before, 11.4 and 11.0 ms after). `--hooks status` adds an `advice` line; installs read `script_outdated` until Install hooks is run once. Script sha256 `df54677f...`; the 6.53.0 script (`1158bb06...`) and this release's pre-QA build (`c0b41bf8...`) can still stop desk runs until then.
- Tests: a real-process end-to-end suite (churn, idle, stubborn, graceful, orphan-stdout three ways, group-orphan, setsid escape; macOS) fails 5 of 9 on 6.53.2 and passes here; queue-turn evidence is tested against the real queue; hook tests run the real script (EPIPE, marker-clear and drain cases fail on the old script). Mutation gate: see the /qa round below.
- **/qa round 1 (before publish).** Four reviewers; one blocker, fixed with the warnings that touch the server:
  - **A `ps` that fails never releases a fence (blocker).** The production liveness probe read every `execFileSync` error as "not running", so a `ps` killed at its 2 s timeout, an `EAGAIN` fork failure or a missing binary released a cancel fence whose child might still be writing. Only the measured "no such process" answer (exit 1, no signal, not killed, no error code, nothing on stdout or stderr) counts as gone now; everything else, and a clean exit that printed nothing, is unverifiable and keeps the fence. `ps` is `/bin/ps` when present.
  - **The halt re-arm matches the delivered prompt exactly.** It matched the first 120 characters with `includes`, so a short Continue ("yes") matched the person's own next desk prompt: their run was stopped "Cancelled from COS" and the cancelled COS turn then ran. Now the words must be equal (whitespace collapsed, Claude Code's peer-message wrapper taken off), and the first prompt after the hand-off decides: a different prompt spends the re-arm and is logged. Stated limit: if the person's own prompt reaches the session first, the queued COS turn is not stopped; an identical desk prompt typed after the send and taken first still re-arms. An unverified hand-off holds its re-arm 2 minutes, not an hour.
  - **The hook drains stdin on every event.** PermissionRequest, Stop, PostToolUse, SessionStart and SessionEnd read only 1 MiB and left the rest unread, and the spool guard exited before reading; a larger payload could EPIPE the writer. The header comment no longer says the PreToolUse branch writes no spool (it spools AskUserQuestion, ExitPlanMode and prompts).
  - **The mutation gate needs a failing test.** A kill was any non-zero vitest exit; now a mutant is killed only when a named test fails, and a run that errors without one fails the gate. Every 6.53.x case was re-run under that rule on a green baseline: 137 of 137 killed by a failing test (74 from this release, 16 added in this round, 47 from 6.53.0), none errored. The one first-run survivor, a `ps` answer with no exit status, now has its test.
  - **Every server suite removes its temp folders.** 39 test files made `mkdtemp` folders and never removed them; one run of those files left 581 entries in `$TMPDIR` before and none made by a test after. The test config removes its per-run data home too. Folders earlier runs left are not touched.
  - **Honesty.** The `reaped` field doc and the process-tree bullet above are scoped to cancel and timeout; the graceful end-to-end bound is derived from the fixture's stop delay and `TREE_POLL_MS` (2.4 s, was 4 s); the closed-session test imports its retention constant.
  - Not in this round (app or site): auto-finish waiting forever, Past days leaving a live meeting, the app's `nothing_to_save` handling, the app mutation gate's three refused cases, gotcos.com's version lines.
